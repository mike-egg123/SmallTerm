  ...
  <div>
    <DiamondHeader :title="title">
      ...
      <div class="doc_info" slot="middle">
        文档创建者&nbsp;:&nbsp;<li>{{checkArticleInfo.author}}</li>
        创建时间&nbsp;:&nbsp;<li>{{formatDate(checkArticleInfo.created_time)}}</li>&nbsp;&nbsp;
        修改时间&nbsp;:&nbsp;<li>{{formatDate(checkArticleInfo.updated_time)}}</li>&nbsp;&nbsp;
      </div>
      <!-- 三个小按钮 -->
    ...
    <el-container style="margin: 0 100px">
      ...
      <el-main>
        <div style="height:auto!important; min-height:580px;">
        ...
        <!-- 评论区翻页 -->
        <div v-show="this.comments.length>5" style="margin-top: 30px">
        <el-button type="primary" icon="el-icon-caret-left" @click="decrement" round size="mini"></el-button>
        <el-button size="mini">第{{end/5}}页</el-button>
        <el-button type="primary" icon="el-icon-caret-right" @click="increment" round size="mini"></el-button>
        </div>
      </el-main>
    </el-container>
  </div>
...
import {reqReleaseLock, reqGetPermission, reqSetPermission, reqGetAllTeam,
  reqGetPermissionSetting,reqUpdate } from '../../api'
...
export default {
  ...
  mounted(){
    console.log(this.userInfo)//有值
    console.log(this.checkArticleInfo)
    this.title=this.checkArticleInfo.title
    this.content=this.checkArticleInfo.content?this.checkArticleInfo.content:""
  },

  computed:{
    ...mapState(['userInfo','checkArticleInfo']),
    //true:作者本人,能够修改文档权限
    isAuthor(){
      return this.checkArticleInfo.authorid === this.userInfo.userid
    },
    //管理所有的事件
    ...mapGetters(['begin', 'end', 'user'])
  },
  methods:{
    goTo_PersonInfo(){
      this.$router.replace('/personInfo')
    },
    ...mapActions(['increment', 'decrement', 'clickAsync', 'getUserInfo','getLikeList',
      'getGarbageList','getArticleInfo','getCreateList','getCurrentList','recordArticleInfo']),
    //收藏
    star() {
        //调用收藏接口，修改数据库收藏列表
        const result=reqLikeornotArticle(this.userInfo.userid,this.checkArticleInfo.articleid,true)
        this.$message({
          type:'success',
          message:'收藏成功'
        })
    ...
    link () {
      const id='/'+this.checkArticleInfo.articleid
      this.invitelink = location.href.replace(this.$route.path+this.$route.params,'');
      this.$alert('请通过复制以下链接来分享文档'+this.invitelink+id , '分享', {
        confirmButtonText: '复制链接',
        callback: () => {
          this.$copyText(this.invitelink+id).then(
            function(e) {
      ...
      this.$prompt('请输入评论(不多于50字)', '评论该文档', {
        ...
        inputValidator: function (value) {
          ...
          if (!this.isAuthor) {//不是作者本人，才发消息
            const sendRes = reqCommentMessage(this,this.checkArticleInfo.articleid,this.userInfo.userid)
            console.log(sendRes)
          }
          this.$message({
            type: 'success',
    ...
    async addComment(value) {
      const result3 = await reqAddComment(this.checkArticleInfo.articleid,this.userInfo.userid,value)
      console.log(result3)
      const result2 = await reqGetComment(this.checkArticleInfo.articleid)
      console.log(result2)
    ...
    async changePermission() {
      if (this.selectedOptions.length == 0) {
        //没选新的，原样返回
        console.log("Set permission to: " + this.checkArticleInfo.articleid)
      }
        ...
        if (this.selectedOptions[0] == 0 && this.selectedOptions.length > 1) {
          ...
          this.$message('只能选择一种权限')
            ...
            if (this.state == 0) {//私有，teamlist为空
              const result1 = await reqSetPermission(this.userInfo.userid,this.state,this.checkArticleInfo.articleid,[])
              console.log("Set permission to: " + this.checkArticleInfo.articleid)
              ...
              const result1 = await reqSetPermission(this.userInfo.userid,this.state,this.checkArticleInfo.articleid,teamlist)
              console.log("Set permission to: " + this.checkArticleInfo.articleid + result1.status)
            }
        }
      }
    },
    ...
    async change() {
      ...
      console.log(this.checkArticleInfo)
      const result = await reqUpdate(this.checkArticleInfo.articleid,this.userInfo.userid,this.title,this.content,this.permission)
      //保存在checkArticleInfo
      await this.getArticleInfo(this.checkArticleInfo.articleid)
      //修改权限
    ...
    async fresh() {
      ...
      await reqReleaseLock(this.checkArticleInfo.articleid)
      //判断权限
      ...
      //获取历史权限设置
      const Got = await reqGetPermissionSetting(this.userid,this.checkArticleInfo.articleid)
      ...
      //读取文档
      if (this.getArticleInfo.status == 2) {
      ...
      //初始化评论区（一定要先执行）
      const Coms = await reqGetComment(this.checkArticleInfo.articleid)
      this.comments = Coms
      //释放锁，使得查看文档时再次获取
    ...
    releaseTest() {
      const res = reqReleaseLock(this.checkArticleInfo.articleid)
      console.log("框架已释放互斥锁")
    },
    handleBack() {
